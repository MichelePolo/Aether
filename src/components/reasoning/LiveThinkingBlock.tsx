...
export function LiveThinkingBlock({ text }: LiveThinkingBlockProps) {
  if (!text) return null;
  return (
    <div className="p-2 rounded bg-disclosure/5 border border-disclosure/30">
      {/* Announced once when thinking starts; the token-streamed text below is
          supplementary and intentionally not a live region (avoids SR spam). */}
      <span role="status" className="sr-only">Assistant is thinking…</span>
