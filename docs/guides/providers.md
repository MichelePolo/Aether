...
**Default selection**: `ProviderRegistry.defaultName()` returns `deps.defaultOverride` (populated from `AETHER_DEFAULT_PROVIDER`, `server/index.ts`) if it names an entry that currently exists; otherwise it falls back through a fixed preference order — gemini, then openai, then anthropic, then ollama, then `fake:default` — picking the first entry found in each transport. openai-compat is deliberately excluded from this fallback chain.
