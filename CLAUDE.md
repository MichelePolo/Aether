...
4. `runDispatchLoop()` streams provider chunks (`text` / `thinking` / `function_call` / `done`) out as **SSE events**, executes MCP tool calls (capped at 10 per dispatch). Each tool call is gated by `BreakpointService` → `auto` (run) or `gate` (await a user approve/reject decision, 60s timeout).
